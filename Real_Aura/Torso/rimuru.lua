...
-- ใช้ค่า Torso และ Rimuru เป็นค่าเริ่มต้นตามที่กำหนด
function VFX.CreateAura(targetPartName, effectName)
    targetPartName = targetPartName or "Torso"
    effectName = effectName or "Rimuru"
    
    local char = player.Character
    if not char then return end
    
    -- 1. ค้นหาชิ้นส่วนในตัวละคร
    local targetPart = char:FindFirstChild(targetPartName)
    if not targetPart then 
        warn("หาชิ้นส่วนไม่พบ: " .. tostring(targetPartName))
        return 
    end

    -- 2. Toggle ปิด หากมีเอฟเฟกต์อยู่แล้ว
    if char:FindFirstChild(effectName) then
        char[effectName]:Destroy()
        return 
    end

    -- 3. Toggle เปิด สร้าง Part รองรับเอฟเฟกต์
    local auraPart = Instance.new("Part")
    auraPart.Name = effectName
...
function VFX.RemoveAura(effectName)
    effectName = effectName or "Rimuru"
    local char = player.Character
    if char and char:FindFirstChild(effectName) then
        char[effectName]:Destroy()
    end
end
