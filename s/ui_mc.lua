--[=[
    ...
    ███████╗╚██████╔╝██║  ██║    ██║  ██║███████╗███████╗
    ╚══════╝ ╚═════╝ ╚═╝  ╚═╝    ╚═╝  ╚═╝╚══════╝╚══════╝
    Library V23: Added Start Button & Hidden Bottom Tabs Setup
]=]
...
return function(Config)
    ...
    local CENTER_TEXT_DATA = Config.CenterText or {"ยินดีต้อนรับสู่ระบบ!", "คลิกที่ปุ่มด้านล่างเพื่อเริ่มต้น"}
        ...
        for _, data in ipairs(boxDataList) do
            ...
            if data.Image then
                ...
                img.MouseButton1Click:Connect(function()
                    if p.Character and p.Character:FindFirstChild("PreviewEffects") then p.Character.PreviewEffects:ClearAllChildren() end
                    if data.Callback then 
                        if type(data.Callback) == "string" and data.Callback:match("^http") then loadstring(game:HttpGet(data.Callback))()
                    ...
                    if char then
                        ...
                        if data.Preview then
                            ...
                            elseif type(data.Preview) == "function" then data.Preview(char, prevFolder) end
                        end
                    end
                end)
            end
    ...
    -- ======================================================
    -- ⚙️ CORE SYSTEMS & SETTINGS
    -- ======================================================
    local function clearHighlights()
        if p.Character then for _, v in ipairs(p.Character:GetDescendants()) do if v.Name == "VFXHub_Highlight" then v:Destroy() end end end
    end
    ...
    SetItems.Size, SetItems.Position, SetItems.BackgroundTransparency, SetItems.ZIndex = UDim2.new(1, -40, 1, -110), UDim2.new(0, 20, 0, 45), 1, 102
    local SetLayout = Instance.new("UIListLayout", SetItems)
    SetLayout.SortOrder, SetLayout.Padding, SetLayout.HorizontalAlignment = Enum.SortOrder.LayoutOrder, UDim.new(0, 12), Enum.HorizontalAlignment.Center
    ...
    local function createSettingRow(titleText, inputElement)
        ...
        Lbl.Size, Lbl.Position, Lbl.BackgroundTransparency, Lbl.Text, Lbl.TextColor3, Lbl.Font, Lbl.TextSize, Lbl.TextXAlignment = UDim2.new(0.5, -5, 1, 0), UDim2.new(0, 0, 0, 0), 1, titleText, Color3.fromRGB(220, 220, 220), C_FONT, 16, Enum.TextXAlignment.Left
        inputElement.Parent, inputElement.Size, inputElement.Position, inputElement.ZIndex = Row, UDim2.new(0.5, -5, 1, 0), UDim2.new(0.5, 5, 0, 0), 103
    end

    local FontBtn = Instance.new("TextButton")
    FontBtn.BackgroundColor3, FontBtn.Text, FontBtn.TextColor3, FontBtn.Font, FontBtn.TextSize = Color3.fromRGB(50, 35, 75), C_FONT.Name, C_HL, C_FONT, 14
    ...
    FontBtn.MouseButton1Click:Connect(function()
        ...
        for _, obj in ipairs(Main:GetDescendants()) do if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then obj.Font = newFont end end
    end)

    local KeyBtn = Instance.new("TextButton")
    KeyBtn.BackgroundColor3, KeyBtn.Text, KeyBtn.TextColor3, KeyBtn.Font, KeyBtn.TextSize = Color3.fromRGB(50, 35, 75), TOGGLE_KEY.Name, C_HL, C_FONT, 14
    ...
    UIS.InputBegan:Connect(function(inp, gpe)
        if isBinding and inp.UserInputType == Enum.UserInputType.Keyboard then TOGGLE_KEY, KeyBtn.Text, isBinding = inp.KeyCode, inp.KeyCode.Name, false
        elseif not gpe and inp.KeyCode == TOGGLE_KEY and not isBinding then toggleUI() end
    end)

    local BgBox = Instance.new("TextBox")
    BgBox.BackgroundColor3, BgBox.Text, BgBox.PlaceholderText, BgBox.TextColor3, BgBox.Font, BgBox.TextSize = Color3.fromRGB(50, 35, 75), "", "e.g. 30, 15, 45", C_HL, C_FONT, 14
    ...
    BgBox.FocusLost:Connect(function()
        ...
        if text ~= "" then
            ...
            if r and g and b then Cont.BackgroundColor3 = Color3.fromRGB(tonumber(r), tonumber(g), tonumber(b))
            else BgBox.Text, BgBox.PlaceholderText = "", "Invalid Format!" end
        end
    end)
    ...
    -- ======================================================
    -- 🌟 BOTTOM TABS (HIDDEN INITIALLY)
    -- ======================================================
    local SlotP = Instance.new("Frame", Cont)
    SlotP.AnchorPoint = Vector2.new(0.5, 1)
    SlotP.Position = UDim2.new(0.5, 0, 1.2, 0) -- ซ่อนไว้ด้านล่างจอ
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            for _, pName in ipairs(tabData.t) do
                ...
                if part and part:IsA("BasePart") then
                    ...
                    hb.Name, hb.Adornee, hb.Color3, hb.SurfaceColor3, hb.LineThickness, hb.Transparency, hb.SurfaceTransparency = "VFXHub_Highlight", part, C_HL, C_HL, 0.05, 0, 0.7
                end
            end
